...
function updateContext( context: ContextType, routeMatch: RouteMatch<string>[] | null, store: StoreRecord ){
    context.status = routeMatch ? 200 : 400
    context.preloadedState = store.getState()
}

async function getServerData(routeMatch: RouteMatch<string>[] | null, dispatch: Dispatch){
  if(routeMatch){
